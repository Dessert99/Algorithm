...
for (let t = 0; t < T; t++) {
    ...
    while (maxHeap.heap.length > 0) {
        const maxNum = maxHeap.extractMax();
        if (map.get(maxNum) > 0) {
            max = maxNum; // 최댓값 갱신
            break;
        }
    ...
    while (minHeap.heap.length > 0) {
        const minNum = minHeap.extractMin();
        if (map.get(minNum) > 0) {
            min = minNum;
            break;
        }
...
1. 💡 이중 우선순위 큐
    ...
    ```javascript
        // 우선순위 큐
        class PriorityQueue {
            // compare(a, b)가 음수라면 a의 우선순위가 b보다 높다는 뜻이다.
            ...
            heapifyUp() {
                ...
                while (curIndex > 0) {
                    ...
                    // 종료 조건: 부모가 현재 값보다 우선순위가 높거나 같을 때
                    if (this.compare(this.heap[parentIndex], this.heap[curIndex]) <= 0) {
                        break;
                    }
        
                    // 아직 현재 값의 우선순위가 부모보다 높다면 올려야 한다.
                    this.swap(parentIndex, curIndex);
                    curIndex = parentIndex;
                }
            }
            ...
            pop() {
                ...
                // 힙에 값이 하나라면 그게 우선순위가 가장 높은 값이다.
                if (this.heap.length === 1) {
                    return this.heap.pop();
                }
        
                // 우선순위가 가장 높은 값 빼놓기 (맨 앞에 있는 요소)
                const top = this.heap[0];
        
                // 전체 힙 조건을 복구하기 위해서 맨 밑 요소를 루트로 끌어당긴다.
                this.heap[0] = this.heap.pop();
            ...
            // 아래로 내리며 힙 조건 복구
            heapifyDown() {
                // 복구 대상 인덱스. (초기값은 아까 루트로 올렸던 맨 마지막 값 인덱스)
                let curIndex = 0;
        
                // 제자리를 찾아갈 때까지 반복
                while (true) {
                    // 일단 현재 인덱스가 우선순위가 가장 높다고 가정한다.
                    let priorityIndex = curIndex;
                    // 왼쪽 자식 인덱스
                    ...
                    if (
                        ...
                        this.compare(this.heap[leftChildIndex], this.heap[priorityIndex]) < 0
                    ) {
                        // 왼쪽 자식의 우선순위가 더 높다면, 우선순위 인덱스를 바꾼다.
                    ...
                    if (
                        ...
                        this.compare(this.heap[rightChildIndex], this.heap[priorityIndex]) < 0
                    ) {
                        priorityIndex = rightChildIndex;
                    }
        
                    // 비교 이후에도 우선순위 인덱스가 바뀌지 않았다면 지금 위치가 적절하다는 의미이므로, 반복문을 종료한다.
                    if (priorityIndex === curIndex) {
                        break;
                    }
        
                    // 반복문이 종료되지 않았다면, 위치 조정이 필요하다는 의미이다.
                    this.swap(priorityIndex, curIndex);
                    curIndex = priorityIndex;
                }
            }
        }
